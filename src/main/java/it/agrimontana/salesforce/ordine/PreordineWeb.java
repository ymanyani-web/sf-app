...
public class PreordineWeb extends Preordine {
  private static final Logger logger = Logger.getLogger(PreordineWeb.class);
  ...
  public PreordineWeb(String id, SalesforceService salesforceService, PreorderService preorderService,
    ...
    logger.debugf("Nuova istanza oggetto: %s, con id: %s", PreordineWeb.class.getSimpleName(), id);
  }
  ...
  public SmiwconOrdine structSmiwconOrder() {
    logger.debugf(
        "Costruzione dto smiwcon, funzione: %s, tipologia: W, ordine: %s",
        "structSmiwconOrder",
        preorder.getId()
    );
    // righe ordine
    ...
    for (PreorderLineItem item : this.preorderlineitem) {
      logger.debugf("Preordine web riga ordine, codice prodotto: %s, tipo movimento: %s, qta: %s, preordine: %s",
  ...
  public void createSalesforceOrder() {
    ...
    this.salesforceOrder.setOrderType__c("W");
  ...
  public void sendSalesforceOrder() {
    logger.debugf(
        ...
        PreordineWeb.class.getSimpleName(),
        "sendSalesforceOrder"
    );
    ...
    for (PreorderLineItem item : this.preorderlineitem) {
      logger.debugf(
          "Riga web: %s, codice prodotto: %s, id prodotto: %s, id listino: %s",
          item,
          item.getProductCode__c(),
          item.getProduct(),
    ...
    for (SalesforceOrderLines line : this.salesforceOrderLines) {
      ...
      line.setERPKey__c("W-" + salesforceOrder.getName() + "-" + line.getNumeroRigo__c());
  ...
  public CompositeSubRequest preOrderPatch() {
    ...
    preorderBody.put("ERPKey__c", "W-" + salesforceOrder.getName());
  ...
  public CompositeSubRequest orderPatch() {
    ...
    orderPatch.setUrl("/services/data/" + super.salesforceService.getApiVersion() + "/sobjects/Order/ERPKey__c/W-" + salesforceOrder.getName());
