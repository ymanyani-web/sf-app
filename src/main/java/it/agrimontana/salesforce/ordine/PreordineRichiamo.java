...
public class PreordineRichiamo extends Preordine {
  private static final Logger logger = Logger.getLogger(PreordineRichiamo.class);
  ...
  public PreordineRichiamo(String id, SalesforceService salesforceService, PreorderService preorderService,
    ...
    logger.debugf("Nuova istanza oggetto: %s, con id: %s", PreordineRichiamo.class.getSimpleName(), id);
  }
  ...
  public void getDataFromSalesforce() {
    ...
    // Recupero agente o capo area con chiamata separata
    this.agent = fetchAgentOrCapoArea(this.account);
    ...
    // Validazione specifica per Richiamo
    if (agent == null)
      throw new IllegalStateException("Agente non trovato");
    if (agent.getErpKey() == null)
      throw new IllegalStateException("ERPKey agente mancante");
  }

  public SmiwconOrdine structSmiwconOrder() {
    logger.debugf(
        "Costruzione dto smiwcon, funzione: %s, tipologia: R, ordine: %s",
        "structSmiwconOrder",
        preorder.getId()
    );
    ...
    for (PreorderLineItem item : this.preorderlineitem) {
      logger.debugf("Preordine richiamo riga ordine, codice prodotto: %s, tipo movimento: %s, qta: %s, preordine: %s",
      ...
      if ("31".equals(tipoRigo)) {
        ...
        // Vendita
        tipoMovimentoAcronimo = (Double.parseDouble(item.getDiscount4__c()) != 0) ? "BV" : "VE";
      } else {
        // Altri casi
      ...
      SmiwconRigheOrdine row = new SmiwconRigheOrdine.Builder()
          ...
          .flagSconto5("%")
          .numeroRigaOfferta(
              String.valueOf((int) Double.parseDouble(item.getNumeroRigo()))
          )
          .numeroOfferta(item.getSourceOrderItem__r(), item.getId())
          .tipoOfferta("P")
          .build();
      rows.add(row);
      numeroRiga++;
    ...
    SmiwconOrdine order = new SmiwconOrdine.Builder()
        ...
        .tipologiaDiVendita2("LABO")
        .noPark("S")
        .agente(agent.getErpKey())
        .pagamento(preorder.getPaymentMethod__c(), account.getPreferredPaymentMethod())
  ...
  public void createSalesforceOrder() {
    ...
    this.salesforceOrder.setIsBooking__c(false);
    this.salesforceOrder.setAccountId(this.account.getId());
    this.salesforceOrder.setOrderType__c("R");
  ...
  public void sendSalesforceOrder() {
    logger.debugf(
        ...
        PreordineRichiamo.class.getSimpleName(),
        "sendSalesforceOrder"
    );
    ...
    for (SalesforceOrderLines line : this.salesforceOrderLines) {
      ...
      line.setERPKey__c("R-" + salesforceOrder.getName() + "-" + line.getNumeroRigo__c());

      // sostituisci codice erp con id salesforce
  ...
  public CompositeSubRequest preOrderPatch() {
    ...
    preorderBody.put("ERPKey__c", "R-" + salesforceOrder.getName());
  ...
  public CompositeSubRequest orderPatch() {
    ...
    orderPatch.setUrl("/services/data/" + super.salesforceService.getApiVersion() + "/sobjects/Order/ERPKey__c/R-" + salesforceOrder.getName());
